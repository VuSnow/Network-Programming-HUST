...
void printScoreBoardToFile(const ScoreBoard* scoreBoard) {
    ...
    while (currentStudent != nullptr) {
        outFile << "S|" << currentStudent->studentID << "|" << currentStudent->firstName << "|"
                << currentStudent->lastName << "|" << currentStudent->progress << "|"
                << currentStudent->finalExam << "|" << currentStudent->grade << "|" << std::endl;

        currentStudent = currentStudent->next;
        studentCount++;
    }

    outFile << "StudentCount|" << studentCount << std::endl;
    outFile.close();
    std::cout << "Scoreboard for " << scoreBoard->subjectID << " in semester " << scoreBoard->semester << " written to file." << std::endl;
}
